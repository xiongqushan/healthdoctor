...
import haozuo.com.zxinglib.camera.CameraManager;
...
public final class CaptureActivityHandler extends Handler {
	...
	public void handleMessage(Message message) {
		switch (message.what) {
		case ZXConfig.restart_preview:
			restartPreviewAndDecode();
			break;
		case ZXConfig.decode_succeeded:
			state = State.SUCCESS;
		...
		case ZXConfig.decode_failed:
			...
			cameraManager.requestPreviewFrame(decodeThread.getHandler(),
					ZXConfig.decode);
			break;
		case ZXConfig.return_scan_result:
			activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
			activity.finish();
			break;
		case ZXConfig.launch_product_query:
	...
	public void quitSynchronously() {
		...
		Message quit = Message.obtain(decodeThread.getHandler(), ZXConfig.quit);
		quit.sendToTarget();
		try {
		...
		removeMessages(ZXConfig.decode_succeeded);
		removeMessages(ZXConfig.decode_failed);
	}
